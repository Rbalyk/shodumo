...
/** Inline-SVG icon set (ported 1:1 from the legacy design). */
...
/** Build a raw inline-SVG markup string for an icon (for non-component DOM, e.g. Leaflet popups). */
export function iconSvg(name: IconName, opts: IconSvgOptions = {}): string {
  const { size = 22, stroke = 2, fill = false } = opts;
